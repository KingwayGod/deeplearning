...
def getPrediction( args, groundTruthFile ):
    ...
    if not args.predictionPath:
        ...
        if 'CITYSCAPES_RESULTS' in os.environ:
            ...
            #配置自定义路径
            rootPath = os.path.join("/content","results")
...
#----------------------------groundTruth file---------------------------------
args.groundTruthSearch  = os.path.join( "/content" , "gtFine" , "val" , "*", "*_gtFine_labelTrainIds.png" )
...
def evaluatePair(predictionImgFileName, groundTruthImgFileName, confMatrix, instanceStats, perImageStats, args):
    ...
    try:
        ...
        groundTruthImg=groundTruthImg.resize((256,256),Image.NEAREST) #(for encoder)
